...
opt = tf.keras.optimizers.Adam(learning_rate=0.1) #경사하강법을 이용하여 변수 값을 변경해줌 ()를 비우면 자동으로 지정 ()안에 수동으로 지정도 가능

#opt.minimize(손실함수, var_list=[a,b]) #경사하강 1번 해줌 == a,b를 1번 수정해줌

#반복할 수록 좋은 값이 도출된다
for i in range(300):
    opt.minimize(손실함수, var_list=[a,b]) #var_list는 경사하강법으로 업데이트할 weight Variable 목록
